...
const MobileNav = () => {
  ...
  return (
    <nav className="md:hidden flex-grow flex flex-col">
      ...
      <div className="toggleable-nav bg-offWhite text-md h-full flex-grow flex flex-col px-8 pt-8">
        <Link href="/" className="border-b-[1px] py-5">Mission</Link>
        ...
        <div>
          ...
          <div className={`nav-dropdown transition-ease duration-[1000ms] ${eyeglassesOpen ? "flex" : "hidden"}`}>
            <Link href="/">{`Men's`}</Link>
            <Link href="/">{`Women's`}</Link>
            <Link href="/">{`Materials`}</Link>
          </div>
        ...
        <Link href="/" className="border-b-[1px] py-5">Stockists</Link>
      </div>
    </nav>
  );
};
